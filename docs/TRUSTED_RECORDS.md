...
## Snapshot-gated sharing
Persistent share creation calls a single policy service. Healthcare and legal meetings require an eligible published snapshot; compatibility-mode general meetings retain existing behavior. Share records store nullable snapshot and policy-version identifiers so public content can remain tied to an immutable publication.
